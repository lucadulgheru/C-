...
class Graph {
    public:
        int N;
        // tableau de listes! (une liste pour chaque sommet)
        LinkedList<struct list_elem_info<TedgeInfo> > *L;

        //tableau pour l'info des noeuds
		TnodeInfo *nodeInfo;
...
// remove the element pointed to by p from L[i] -- code ommitted
                ...
                L[i].removeFirstOccurrence(p->info);
...
// remove the element pointed to by p from L[j] -- code ommitted
        ...
        L[j].removeFirstOccurrence(p->info);
        }
